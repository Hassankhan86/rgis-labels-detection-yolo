"""
Materializes dataset3's oversample as an actual standalone dataset,
dataset4/ -- same idea as 5-balance_dataset.py's train_oversampled.txt (a
...
than a list of repeated paths back into dataset3. Use this version if you
want dataset4/ to be a self-contained, inspectable dataset (e.g. to hand
...
dataset3/README.txt (the 6 test images must never be used for training)
carries over untouched.

Output layout (nc=4, matches dataset3/classes.txt: small/medium/large/price_label):
    dataset4/images/{train,valid,test}/*.jpg
    dataset4/labels/{train,valid,test}/*.txt
    dataset4/classes.txt
    dataset4/data.yaml
    dataset4/README.txt
...
def main() -> None:
    ...
    parser.add_argument("--source", type=Path, default=Path(__file__).resolve().parent / "dataset3")
    parser.add_argument("--out", type=Path, default=Path(__file__).resolve().parent / "dataset4")
    parser.add_argument(
        "--max-repeat",
        type=int,
        default=6,
    ...
    readme.write_text(
        f"dataset4 -- class-balanced oversample of {source.name}, YOLO-detect format\n"
        f"{'=' * 70}\n"
