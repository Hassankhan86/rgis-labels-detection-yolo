...
## Status update #2 — both Step 1 and Step 3 shipped; still >20min for a 10s clip

Both changes above have since actually been implemented and tested on the
phone: frame-skipping (`frameStep=3`) is live in
`video_batch_repository_impl.dart`, and the int8 model is in
`assets/models/model.onnx`. Result: a 10-second recorded video still takes
**20+ minutes** in the "detecting" phase. Neither change made a noticeable
dent.

**That itself is the key diagnostic signal.** Both changes only affect the
*model inference* call. If inference were the bottleneck, skipping 2/3 of
inference calls should have cut time roughly 3x, and int8 should have
mattered too. Getting ~0 improvement from either means **inference was
never the dominant cost** — something that runs on *every* frame regardless
of `frameStep` or model choice is.

Tracing the per-frame loop in `video_batch_repository_impl.dart` confirms
this: `img.decodePng(await srcFile.readAsBytes())` and
`img.encodePng(decoded)` run unconditionally on every extracted frame —
they're outside the `frameStep` gate entirely. Both are pure-Dart, not
hardware-accelerated codecs (`package:image`), and they operate on the
frame at **the source video's native recording resolution** — `ffmpeg_frame_extractor.dart`'s
extraction command (`extractFrames`) has no scale filter at all today, so a
1080p+ phone recording gets extracted as ~300 full-resolution PNGs. Pure-Dart
PNG decode+encode at that resolution, done 300 times sequentially, is
consistent with a 20+ minute run. `frame_annotator.dart`'s drawing calls
(`drawRect`/`fillRect`/`drawString`) are cheap by comparison — small,
localized operations, not full-frame.

This directly answers the two questions on the table:
- **Increasing `frameStep` further**: not the right lever right now —
  inference isn't the bottleneck, so skipping more of it won't move the
  number much. Leave it at `3` for now.
- **Trying "640 resolution"**: right instinct, wrong knob. The model's
  `imgsz` (960) is unrelated to this — retraining/re-exporting the model
  wouldn't touch the PNG decode/encode cost at all, since that cost comes
  from the *source video frame* size, not the model's input size. The fix
  is downscaling at **extraction** time (Step 4 below), which is free
  (ffmpeg's own scaler, not Dart) and doesn't require retraining anything.
...
### Step 4 — Downscale frames at extraction time (NEW — this is the actual fix now)

`ffmpeg_frame_extractor.dart`'s `extractFrames` builds this command today:

```
-hide_banner -y -i "$sourceVideoPath" -vsync 0 "$pattern"
```

Add a scale filter so ffmpeg (native, SIMD-accelerated) downsizes each
frame *before* it's written to disk, instead of every later Dart-side step
paying for full-resolution pixels:

```
-hide_banner -y -i "$sourceVideoPath" -vf "scale='min(960,iw)':'min(960,ih)':force_original_aspect_ratio=decrease:force_divisible_by=2" -vsync 0 "$pattern"
```

- `min(960,iw)`/`min(960,ih)` + `force_original_aspect_ratio=decrease`: fit
  within a 960x960 box, preserving aspect ratio, never upscaling a
  already-smaller source.
- `force_divisible_by=2`: keeps width/height even, which the re-encode step
  (`ffmpeg_video_encoder.dart`, `-pix_fmt yuv420p`) requires.
- 960 matches `AppConstants.modelInputSize`/the model's own working
  resolution, so `letterboxResize` needs to scale less (or not at all)
  before feeding the model — no detection-quality loss, since the model
  never sees more detail than 960px on a side anyway.
- This single change should cut per-frame decode+encode cost roughly by
  the square of the resolution ratio — e.g. ~4x fewer pixels if the source
  is 1080p, likely more for 4K.

No Dart-side changes needed elsewhere: `letterboxResize` already computes
its scale/padding dynamically from whatever size it's handed, and the
output video's resolution will simply follow the (now smaller) annotated
frame size automatically.

**If this alone isn't enough**, in order of effort:
- Add lightweight `Stopwatch` timing around extraction / decode / infer /
  encode+write in `video_batch_repository_impl.dart`'s loop, temporarily,
  to get hard numbers instead of continuing to infer the bottleneck from
  behavior — especially useful if Step 4 helps but doesn't fully close the
  gap, to see what's left.
- Swap the intermediate frame format from PNG to JPEG (`img.decodeJpg`/
  `img.encodeJpg`, and the `.png` pattern/extension in both
  `ffmpeg_frame_extractor.dart` and `ffmpeg_video_encoder.dart`) — lossy
  compression is irrelevant here since these are throwaway intermediate
  files, and JPEG encode is typically markedly faster than PNG's deflate-based
  compression for the same pixel count.
- Parallelize across frames (process frame N+1's decode while frame N is
  mid-inference, or fan out across isolates) — a bigger architectural
  change, only worth it if Steps 1-4 combined still aren't fast enough.
...
### If Steps 1-4 still aren't enough
...
- `flutter analyze` and `flutter test` after each code change (Step 1,
  already done; Step 4, once implemented).
- Manually run the *same* short recorded video through the batch-process
  tab before/after Step 4 on the connected phone, and time the "detecting"
  phase wall-clock — this is the number that should finally visibly move.
  Confirm the output video still looks reasonable at the new (smaller)
  resolution and that unique-label counts are in the same ballpark as
  before (tracker/counter logic is untouched by Step 4).
- If Step 4 helps but the number is still far off, that's the point to add
  the `Stopwatch` instrumentation mentioned under Step 4's fallback list
  rather than guessing at the next change.
